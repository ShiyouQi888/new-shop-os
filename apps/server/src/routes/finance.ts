...
router.get('/overview', (_req, res, next) => {
  try {
    ...
    const serviceFee = get<{ v: number }>('SELECT COALESCE(SUM(service_fee),0) AS v FROM resell_order WHERE status IN (0,1)')!.v
    ...
    ok(res, {
      orderIncome, serviceFee, commissionOut, withdrawOut, pendingWithdraw,
      net, // 平台净收入
    })
  } catch (e) { next(e) }
})
